...
class ContactFinder extends SearchDelegate<Contato>
{
  ...
  List<Widget> buildActions(BuildContext context) {
      return is_on_result_screen ? [] : [IconButton(icon: Icon(Icons.clear), onPressed: () {query = '';})];
    }
      ...
      Column(
        children: [
          ...
          Row(
            ...
            children: [
              CircleAvatar(child: Text(c.name[0], style: TextStyle(fontSize: 30)), radius: 40,),
              SizedBox(width:20),
              Column(children: [
    ...
    return Consumer<ContatosProvider>(
      builder: (context, contas, _) {
        List<Contato> filtered_contacts = [];

        if(query.isNotEmpty)
        {
          filtered_contacts = contas.all_inserted.where((element) => element.name.toUpperCase().startsWith(query.toUpperCase())).toList();
        }
        else
        {
